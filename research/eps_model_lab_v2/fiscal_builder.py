"""V2-owned fork of V1 fiscal construction. Patched through explicit V2 callbacks.
Original bytes are read-only; all production builds install V2 context selectors.
"""
...
from research.eps_model_lab_v2.common import RUN,V1,save_json,sha
RAW_RUN = V1
...
def one(ticker,companyfacts_override=None):
    dest=RAW_RUN/'data/raw/sec'/ticker
    ...
    pricepath=RAW_RUN/'data/raw/prices'/f'{ticker}.parquet'
    ...
    for accn,values in sorted(native.items(),key=lambda item:(filings[item[0]]['asof'],item[0])):
        f=filings[accn]
        ...
        # A direct quarter must start immediately after the already disclosed
        # prior native fiscal-quarter end. This admits evidenced 13/14/17-week
        # periods without guessing from an approximate day-count bucket.
        prior_ends=[r['end'] for r in records if r['asof']<f['asof'] and r['end']<f['end']]
        expected_quarter_start=(pd.Timestamp(max(prior_ends))+pd.Timedelta(days=1)).strftime('%Y-%m-%d') if prior_ends else None
        if q==1: expected_quarter_start=fiscal_start
        direct=[v for v in values if expected_quarter_start is not None and v['start']==expected_quarter_start]
        ...
        row={**f,'ticker':ticker,'fiscal_quarter':q,'native_fiscal_start':fiscal_start,
             ...
             'eps':eps,'direct_quarter_start':expected_quarter_start,
             'eps_ytd':eps_ytd,
    ...
    try:
        ...
        reference=RAW_RUN/'data/engine_events'/f'{ticker}.parquet'
